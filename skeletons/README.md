...
These are **starting points, not solutions.** The finished services live in the
[course repo](https://github.com/githubmo/modern-java-25-course)'s `apps/`
(`kyc-service`, `screening-service`) and at its `checkpoint/day-NN` Git tags.
...
(The course repo's `apps/postman` collection's `screeningBaseUrl` defaults to `http://localhost:8082` to match.)
...
`../day-10-messaging-capstone/README.md`. Reference app: the course repo's `apps/`.
